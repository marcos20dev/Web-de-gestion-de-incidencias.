...
@section('content')
    <div
        class="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-surface-900 via-surface-800 to-surface-900">
        <!-- Efectos de fondo -->
        <div class="fixed inset-0 overflow-hidden pointer-events-none">
            <div
                class="absolute -top-40 -right-40 w-80 h-80 bg-primary-500 rounded-full mix-blend-multiply blur-3xl opacity-10 animate-[float_6s_ease-in-out_infinite]">
            </div>
            <div
                class="absolute -bottom-40 -left-40 w-80 h-80 bg-primary-400 rounded-full mix-blend-multiply blur-3xl opacity-10 animate-[float_6s_ease-in-out_infinite] animation-delay-[2s]">
            </div>
            <div
                class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-primary-300 rounded-full mix-blend-multiply blur-3xl opacity-5 animate-[float_6s_ease-in-out_infinite] animation-delay-[4s]">
            </div>
        </div>
        ...
        <div class="w-full max-w-7xl mx-auto">
            <div
                class="bg-white/5 backdrop-blur-xl rounded-3xl overflow-hidden shadow-2xl border border-surface-700 shadow-[0_0_20px_rgba(74,222,128,0.15)]">
                <div class="grid grid-cols-1 lg:grid-cols-2 min-h-[700px]">

                    <!-- Lado izquierdo - Branding -->
                    <div
                        class="bg-gradient-to-br from-surface-800/90 to-surface-900/95 p-12 flex flex-col justify-between animate-[slide-in-left_0.8s_ease-out]">
                        <div>
                            <div class="flex items-center space-x-4 mb-8">
                                <div
                                    class="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center shadow-[0_0_20px_rgba(74,222,128,0.15)]">
                            ...
                            <div class="space-y-6">
                                <h2 class="text-4xl font-bold text-white leading-tight">
                                    Únete a Nuestra
                                    <span
                                        class="bg-gradient-to-r from-primary-400 to-primary-500 bg-clip-text text-transparent">Plataforma</span>
                                    de Gestión
                                </h2>

                                <p class="text-surface-300 text-lg leading-relaxed">
                                    Crea tu cuenta y comienza a gestionar incidencias de manera profesional con nuestro
                            ...
                            <div class="bg-surface-800/50 rounded-xl p-4 border border-surface-700">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-surface-400 text-xs">Tu cuenta será activada después de la revisión
                                            administrativa</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <form class="space-y-6" id="registerForm" method="POST" action="{{ route('register') }}">
                                ...
                                <div>
                                    ...
                                    <input id="email" name="email" type="email" required
                                        class="w-full px-4 py-3 bg-surface-800 text-white border border-surface-600 rounded-xl placeholder-surface-500 focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/30 transition duration-200"
                                        placeholder="tu@empresa.com" value="{{ old('email') }}">
                                    @error('email')
                                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <button type="button"
                                                ...
                                                <i
                                                    class="fas fa-eye text-surface-400 hover:text-surface-300 transition duration-200"></i>
                                            </button>
                                        </div>
                                        <div class="mt-2 flex space-x-1">
                                            <div
                                                class="password-strength w-1/4 bg-surface-600 rounded h-1 transition-all duration-300">
                                            </div>
                                            <div
                                                class="password-strength w-1/4 bg-surface-600 rounded h-1 transition-all duration-300">
                                            </div>
                                ...
                                <!-- Selección de Rol -->
                                <!-- Selección de Rol (dinámico desde la base de datos) -->
                                <div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                        @foreach ($roles as $rol)
                                            <div class="role-option border-2 border-surface-600 rounded-xl p-3 text-center cursor-pointer transition-all duration-300 hover:translate-y-[-2px] hover:border-primary-400/30"
                                                ...
                                                <h3 class="font-medium text-surface-200 text-sm">
                                                    {{ ucfirst($rol->nombre) }}</h3>
                                            </div>
                                        @endforeach
                                    </div>

                                    <input type="hidden" id="selected_role" name="rol"
                                        value="{{ old('rol') }}">
                                    @error('rol')
                                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                    ...
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-surface-400 text-xs mt-1">
                                                Después del registro, tu cuenta estará <span
                                                    class="text-primary-400">pendiente de aprobación</span>.
                                                Recibirás un correo cuando sea activada.
                                            </p>
                                            <button type="button" id="showApprovalInfo"
                                                class="text-primary-400 hover:text-primary-300 text-xs mt-2 flex items-center">
                                                <i class="fas fa-info-circle mr-1"></i>
                                                Más información sobre el proceso
                                            </button>
                                            <div id="approvalInfo"
                                                class="max-h-0 opacity-0 overflow-hidden transition-all duration-400 mt-2">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <label for="terms" class="ml-2 block text-sm text-surface-300">
                                        Acepto los <a href="#"
                                            class="text-primary-400 hover:text-primary-300">términos</a> y
                                        <a href="#" class="text-primary-400 hover:text-primary-300">política de
                                            privacidad</a>
                                    </label>
                                </div>
                                @error('terms')
        ...
        document.querySelectorAll('.role-option').forEach(option => {
            option.addEventListener('click', function() {
                document.querySelectorAll('.role-option').forEach(opt => {
                    opt.classList.remove('border-primary-500', 'bg-primary-500/5',
                        'shadow-[0_0_20px_rgba(74,222,128,0.15)]');
                });

                this.classList.add('border-primary-500', 'bg-primary-500/5',
                    'shadow-[0_0_20px_rgba(74,222,128,0.15)]');
